...
export const handleClientProductIncrement = async (productId, setProductCounts, productCounts,userId) => {
  ...
  if (productIndex !== -1) {
    ...
    setProductCounts(updatedProductCounts);
    await updateClientProduct({_id:productId,quantity,userId,update:"quantity",totalPrice})
  } else {
    setProductCounts(prevCounts => [
      ...prevCounts,
      {
...
export const handleClientProductDecrement = async(productId, setProductCounts, productCounts,userId) => {
  const productIndex = productCounts.findIndex(item => item.id === productId);
  if (productIndex !== -1 && productCounts[productIndex]?.quantity > 1) {
    const updatedProductCounts = [...productCounts];
    const quantity= updatedProductCounts[productIndex]?.quantity - 1
