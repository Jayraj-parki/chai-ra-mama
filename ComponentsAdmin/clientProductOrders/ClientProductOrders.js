...
const ClientProductOrders = () => {
    ...
    const { helper, productOrders } = useProductOrdersContext()
    const [otp, setOtp] = useState()
    const updateData = async (_id, status) => {
        setAlert({ modalActive: true, workStatus: "progress", message: "Please wait..." })
        await updateClientOrderStatus({ _id, status, helper, setAlert })
    }
    const sendOtp = async (orderId, email, type) => {
        let isOTPsent = false
        if (type == "otp") {
            setAlert({ modalActive: true, workStatus: "progress", message: "Please wait..." })
            isOTPsent = await sendOrderOTP({ orderId, userEmail: email, setAlert, end_url: "client-orders" })
        }
        else isOTPsent = true
        if (isOTPsent) {
    ...
    const verifyOTP = async ({ orderId, userEmail, inputOtp }) => {
        setAlert({ modalActive: true, workStatus: "progress", message: "Please wait..." })
        await verifyOtpAndDeliver({ orderId, userEmail, inputOtp, setAlert, helper, setOtpVerification, end_url: "client-orders" })
        setOtp("")
    }
                        ...
                        val?.productDetails?.length > 0 ?
                            <>
                                <div key={val?._id + "" + index} className={style.container + ' row col-12 mx-auto py-3 my-3 shadow rounded-3 border '}>
                                    {val?.storeDetails && <div className={style.storeDetails + ' row col-12 mx-auto mb-3'}>
                                        <label className='px-0'>Store Details</label>
                                        <table className="col-12 table table-bordered   text-center text-capitalize mt-2 ">
                                            <thead className='border p-0'>
                                                ...
                                                <th className='text-capitalize  border text-center' >Map</th>
                                            </thead>
                                            <tbody>
                                                {
                                                    val?.storeDetails &&
                                                    <tr key={val?.storeDetails?.storePhone + "" + index + "" + Math?.random(10000)} className=''>
                                                        <td className='align-middle' >{val?.storeDetails?.storeAddress}</td>
                                                        <td className='align-middle' >{val?.storeDetails?.storePhone}</td>
                                                        <td className='align-middle'><button className='btn btn-secondary text-light text-decoration-none text-uppercase' onClick={() => window.open(val?.storeDetails?.storeMap || "", '_blank')}>view map</button> </td>
                                                    </tr>
                                                }
                                            </tbody>
                                        </table>
                    ...
                    {productOrders?.completed_orders?.map((val, index) =>
                        <>
                            <div key={val?._id + "" + index} className={' row col-12 mx-auto py-3 my-2 shadow rounded-3 border'}>
                                {val?.storeDetails && <div className={style.storeDetails + ' row col-12 mx-auto mb-4'}>
                                    <label className='px-0'>Store Details</label>
...
export default ClientProductOrders