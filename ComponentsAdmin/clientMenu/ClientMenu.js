  ...
  return (
    <>
      ...
      <div className={style.clientMenu + ' container-fluid my-4  shadow rounded-4 p-4'}>
        ...
        <ClientMenuEdit editData={editData} setEditData={setEditData} />
        <ClientMenuAdd addData={addData} setAddData={setAddData} />
                    ...
                    clientMenuData?.map((val, index) =>
                      <tr key={index + "" + Math?.random(10000)} className=''>
                        ...
                        <td className='text-center align-middle'>
                          <button onClick={() => setEditData({ active:  true, _id: val?._id, itemName: val?.clientMenuName, itemPrice: val?.clientMenuPrice,itemImage:val?.clientMenuImage  })} className='btn btn-primary text-decoration-none mx-2 text-capitalize'>Edit</button>
