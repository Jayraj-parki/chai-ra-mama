...
import { useProductOrdersContext } from '@/app/admin/product-orders/page';
...
const ProductOrders = () => {
    ...
    const sendOtp = async (orderId, email, type) => {
        ...
        if (type == "otp") {
            setAlert({ modalActive: true, workStatus: "progress", message: "Please wait..." })
            isOTPsent = await sendOrderOTP({ orderId, userEmail: email, setAlert })
        }
        else isOTPsent = true
        if(isOTPsent){
    ...
    return (
        <>
            <div className={style.tabsContainer + "  row col-12 d-flex  my-5 d-flex flex-row mx-auto  justify-content-center  "}>
                ...
                {/* <div className={` row col-12    p-0 mx-auto d-flex justify-content-center align-items-center flexwrap   mb-4 tab-pane fade ${activeTab == "completed" ? "show active" : "d-none"}`} id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    <table className="col-12 table table-bordered table-hover   text-capitalize ">
                        <thead className='border'>
                            <th className='text-capitalize p-2 pb-4 border text-center' >Sr No.</th>
                            <th className='text-capitalize p-2 pb-4 border text-center' >Name</th>
                                        ...
                                        <div className='row col-12 mx-auto mt-3'>
                                            <div>
                                                ...
                                                <div className='row col-12 mx-auto'>
                                                    <button onClick={updateData} type="submit" className="btn btn-primary d-flex col-auto px-4 ms-auto text-center shadow-none justify-content-center text-capitalize">Verify</button>
